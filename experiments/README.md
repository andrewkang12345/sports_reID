...
`scripts/benchmark_reid_models.py` compares ReID representations on the 31 annotated
frames in `groundTruth_AllTracking_ARG_FRA_183303`.
...
python scripts/benchmark_reid_models.py \
  ...
  --device cuda:0 \
  --output experiments/results/clip_plus_transreid.json
```
